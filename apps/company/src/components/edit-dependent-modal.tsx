...
export default function EditDependentModal({
  ...
  useEffect(() => {
    if (dependent) {
      setFormData({
        paydaNo: dependent.paydaNo || '12345678',
        name: dependent.name || 'Yelma Abebe',
        relationship: dependent.relationship || 'Son',
        dob: dependent.dob || '8 - March - 2025',
      });
    }
  }, [dependent]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-lg font-medium">Edit Dependent Form</h2>
          <button
            onClick={onClose}
